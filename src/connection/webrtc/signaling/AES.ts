...
export class AES {
    ...
    private static coefMult(a: number[], b: number[], d: number[]) {
        d[0] = AES.gmult(a[0], b[0]) ^ AES.gmult(a[3], b[1]) ^
            AES.gmult(a[2], b[2]) ^ AES.gmult(a[1], b[3])
        d[1] = AES.gmult(a[1], b[0]) ^ AES.gmult(a[0], b[1]) ^
            AES.gmult(a[3], b[2]) ^ AES.gmult(a[2], b[3])
        d[2] = AES.gmult(a[2], b[0]) ^ AES.gmult(a[1], b[1]) ^
            AES.gmult(a[0], b[2]) ^ AES.gmult(a[3], b[3])
        d[3] = AES.gmult(a[3], b[0]) ^ AES.gmult(a[2], b[1]) ^
            AES.gmult(a[1], b[2]) ^ AES.gmult(a[0], b[3])
    }
    ...
    static str2hex(data: string): Uint8Array {
        ...
        for (let i = 0; i < data.length; i += 2) {
            output[ptr++] = parseInt(data.substring(i, i + 2), 16)
        }
        return output
    }
    ...
    static pad(m: Uint8Array): Uint8Array {
        ...
        paddedMessage.set(
            new Array(remainedLength).fill(remainedLength),
            m.length,
        )
        ...
        cbc: {
            mEncrypt(cipher: AES, m: Uint8Array, iv: Uint8Array) {
                ...
                for (let i = 0; i < m.length; i += AES.blockSize) {
                    prev = cipher.encrypt(
                        AES.xor(m.slice(i, i + AES.blockSize), prev),
                    )
                    output.set(prev, i)
                }
                return output
    ...
    public encString(str: string) {
        ...
        const outBytes = mode.mEncrypt(
            this,
            AES.computeMessage(str),
            this.opt.iv,
        )
        // @ts-ignore
    ...
    private addRoundKey(state: Uint8Array, round: number) {
        for (let c = 0; c < AES.AES_NB; c++) {
            ...
            state[AES.AES_NB + c] ^=
                this.KEY[4 * AES.AES_NB * round + 4 * c + 1]
            state[AES.AES_NB * 2 + c] ^=
                this.KEY[4 * AES.AES_NB * round + 4 * c + 2]
            state[AES.AES_NB * 3 + c] ^=
                this.KEY[4 * AES.AES_NB * round + 4 * c + 3]
        }
    }

    private mixColumns(state: Uint8Array, inverse: boolean = false) {
        const a: number[] = inverse
            ? [0x0e, 0x09, 0x0d, 0x0b]
            : [0x02, 0x01, 0x01, 0x03]
    ...
    private shiftRows(state: Uint8Array, inverse: boolean = false) {
        for (let i = 1; i < 4; i++) {
            ...
            while (s < i) {
                const tmp = inverse
                    ? state[AES.AES_NB * i + AES.AES_NB - 1]
                    : state[AES.AES_NB * i + 0]

                if (inverse) {
                    for (let k = AES.AES_NB - 1; k > 0; k--) {
                        state[AES.AES_NB * i + k] =
                            state[AES.AES_NB * i + k - 1]
                    }
